...
async def checkCounting(ctx):
    if ctx.message.channel.id in getServerDoc(ctx.message.guild.id, ['plugins', 'counting', 'channels']):
        ...
        lastMsg = lastMsg[1]
        if not ctx.message.content.isdecimal() and lastMsg.content.isdecimal() \
        or int(ctx.message.content) != int(lastMsg.content) + 1 \
        or ctx.message.author.id == lastMsg.author.id:
